...
#     behaves like a concurrent caching proxy. Passing these tests
#     does not mean that your proxy is correct.
#
#     updated: 11/18/2013
# 
#     usage: ./driver.sh
...
# usage: download_proxy <testtdir> <filename> <origin_url> <proxy_url>
#
...
# usage: download_noproxy <testtdir> <filename> <origin_url>
#
...
# Make sure we have the right Tiny files with the right permissions
if [ ! -d ./tiny ]
then 
...
if [ ! -x ./proxy ]
then 
    echo "Error: ./proxy not found or not an executable file."
    exit
fi
...
# Stop any background jobs that try to write to the terminal
# stty -tostop
