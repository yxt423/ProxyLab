 ...
 */
 void doit(int connfd){
    rio_t rio_conn, rio_c;
	char *path = NULL;
    int *port = NULL;
	char hostname[MAXLINE],send_req[MAXLINE],response[MAXLINE];
	...
	Rio_readlineb(&rio_c,response,MAXLINE); /* read the server’s response*/
...
int parse_request(rio_t *rio_conn, char *hostname, char *path, int *port, char *send_req) {
	...
	dbg_printf("request from client: %s n = %d\n",request,n);
	
	/* this proxy only support GET */
    sscanf(request, "%s %s %s", method, url, version);
	dbg_printf("parse request: method: %s, url: %s, version: %s",method,url,version);
	...
	dbg_printf("parse request: method: %s, hostname: %s, port: %d, path: %s",method,hostname,*port,path);
